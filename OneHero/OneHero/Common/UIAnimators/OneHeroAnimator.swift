...
final class Animator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    private let secondViewController: AboutCharacterViewController
    private var selectedCellImageViewSnapshot: UIView
    private let cellImageViewRect: CGRect
    private let cellLabelRect: CGRect

    init?(type: PresentationType, firstViewController: MainViewController, secondViewController: AboutCharacterViewController, selectedCellImageViewSnapshot: UIView) {
        self.type = type
        ...
        // 46
        self.cellLabelRect = selectedCell.characterTitle.convert(selectedCell.characterTitle.bounds, to: window)
    }
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        guard
            ...
            let cellImageSnapshot = selectedCell.characterImageView.snapshotView(afterScreenUpdates: true),
            let controllerImageSnapshot = secondViewController.headerCharacterImageView.snapshotView(afterScreenUpdates: true),
            let cellLabelSnapshot = selectedCell.characterTitle.snapshotView(afterScreenUpdates: true), // 47
        ...
        // 25
        let controllerImageViewRect = secondViewController.headerCharacterImageView.convert(secondViewController.headerCharacterImageView.bounds, to: window)
        // 49
        let controllerLabelRect = secondViewController.titleText.convert(secondViewController.titleText.bounds, to: window)
        // 55
